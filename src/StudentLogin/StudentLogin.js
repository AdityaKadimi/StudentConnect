...
// }
    ...
    return (
      <div id ="test">
      <h4 id ="title">StudentConnect</h4>
      <h4 id ="title">Student Dashboard</h4>
      <div id ="tutorid">
         <Link to = "/tutor"><button className="btn btn-primary" id = "btn" id = "tutorid"  disabled = {userType == "S"}>Show Tutor Dashboard</button></Link>
      </div>
    <div className="input-group" id ="search">
      <input className="form-control form-control-lg  width100" placeholder ="Search Services using zipcode or subject name"
          onChange = {this.searchAvailableServices} />
      <span className="input-group-btn" >
      <button className="btn btn-primary" id = "btn" onClick = {this.getAvailableServices}>Search</button>
      <button className="btn btn-primary" id = "btn" onClick = {this.tutorRegister.bind(this,userId)} disabled = {userType == "T" || userType == "B"}>Register as a Tutor</button>
      </span>
        <Link to = '/'><button className="btn btn-secondary" id = "logout" >Logout</button></Link>
      </div>
      <div id = "details">
