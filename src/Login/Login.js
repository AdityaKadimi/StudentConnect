  ...
  registerUser = (registerDetails) => {
    ...
    this.loginService.registerUserData(registerDetails).then(
      response => {
        ...
        alert("User Id created. Please use the ID for login. The user ID:" + response);
        this.setState({
          login:true,
          signup:false
        });
...
<img  width="250" alt="University Logo" src = {images} />
  ...
  <div id="buttons">
        <p id="signupButton"  onClick={this.switch.bind(null,"signup")} className={this.state.signup ? "pink":"blue"}>Register</p>
        <p id="loginButton" onClick={this.switch.bind(null,"login")} className={this.state.login ? "pink":"blue"}> Login</p>
